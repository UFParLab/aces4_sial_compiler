...
public class DecVisitor extends AbstractVisitor implements SialParsersym, SipConstants {
	...
	//To be called by SialCompiler after executing this vistor
	public void createArrayTable(){
		arrayTable.createArrayTable();
	}
	...
	public DecVisitor(SipTable sipTable) {
		this.sipTable = sipTable;
		init(sipTable);
	}

	public DecVisitor(CommandLine options) {
	...
	//priorities are currently ignored in the sip.  
	@Override
	public boolean visit(ArrayDec n) {
		if (n.isUsed()) {
			...
			int priority = 0;
			int kind = n.getArrayKind().getakind().getKind();
			switch (kind) {
			case TK_distributed:{
				priority = SipConstants.distributed_array_priority;
				arrayTable.addDistributedOrServedArrayEntry(n, rank, attribute, indarray, priority);
			} break;
			case TK_served: {
			...
			break;
			case TK_local: {
				if (ASTUtils.isContiguous(n)){
					arrayTable.addContigLocalArrayEntry(n, rank, attribute, indarray, priority);
				}
				else {
					if (n.isAllSimpleIndices()){
					priority = SipConstants.local_all_indices_simple;
					}
					arrayTable.addLocalArrayEntry(n, rank, attribute, indarray, priority);
				}				
			}
			case TK_temp: {
				arrayTable.addTempArrayEntry(n, rank, attribute, indarray, priority);
			}
			break;
			case TK_static: {
				arrayTable.addStaticArrayEntry(n, rank, attribute, indarray, priority);
			}
			break;
			default: {
				assert false: "Missing case in DecVisitor.visit(ArrayDec n)";
			}
			}
	...
	public boolean visit(SubIndexDec n) {
		...
		indexTable.addEntry(n, parentIndex, false /* unused */, CodeGenVisitor.unused, false /* unused */, indexTypeNum);
		return false;
	}
